...
// Command alb-mcp serves networking's read-only Application Load Balancer tools
// over MCP, alongside the knowledge and skills an assistant reads before calling
// them:
//
//	POST /mcp                   Streamable HTTP MCP, stateless
//	GET  /llms-full.txt         Knowledge: what a load balancer is made of
//	GET  /runbooks/<name>.md    Skills: the procedures
//	GET  /healthz               liveness
//
// One process, because networking's capability document names all of those
// URLs. Only /mcp takes a credential; see docs.go for why the documents do not.
//
// The server holds no credential of its own for a customer's project: it reads
// through a client built from the caller's own bearer token. So a tool call can
// never see more than the person who asked, the platform's own access control
// stays the single enforcement point, and there is no standing authority here to
// escalate with.
//
...
// own project would be one prompt injection away from another tenant's load
// balancers. The header is set by the already-authenticated caller.
package main
...
const (
	...
	// so the sentence names the actor and rules out the two things the person
	// who asked would otherwise try.
...
// clientConfig then hangs a project control-plane path off the local API server.
// Every tool call fails with a 401 that reads like the caller's token is bad
// when the deployment is what is wrong; failing at boot puts the error where the
// mistake is.
//
...
func checkControlPlaneEndpoint(cfg *rest.Config) error {
	...
	return fmt.Errorf(
		"control plane endpoint %s is this cluster's own API server: alb-mcp reads Datum project "+
			"control planes, not the cluster it runs in, so the in-cluster fallback is never "+
			"correct. Give the deployment an explicit control plane: mount a kubeconfig naming the "+
			"control plane's address and CA, and point KUBECONFIG at it (or pass --kubeconfig)",
		cfg.Host)
}

// localClusterEndpoint returns the API server address in-cluster config resolves
// to, or "" outside a pod. It mirrors rest.InClusterConfig's own derivation
// rather than calling it, so the check still holds when the projected
...
func run(addr string, baseConfig *rest.Config) error {
	...
	// networking's capability document points the assistant at.
...
// endpoint and CA only — every credential field it might carry is cleared first,
// so the server's own identity can never leak into a caller's read.
func clientConfig(baseConfig *rest.Config, token, project string) (*rest.Config, error) {
	// The project arrives in a header and is interpolated into a URL path, so it
	// is validated before it can reshape that path into another API route.

...
Not here, and deliberately so: the per-location policies that deliver a cell its
own identity. Every resource in the policy above goes to *every* labelled edge
cluster, and a location must reach exactly the one cell that serves it. Karmada
has no primitive joining a resource label to a cluster label, so that needs one
policy per location, selecting one `ServingLocation` by name and targeting
`topology.datum.net/location` on the cluster. Those policies are generated by
NSO's location publisher rather than hand-maintained here — a catalog that
required a pull request per location would recreate the two-records-that-must-
agree problem this design exists to remove. Adding a location selector to the
policy above is not a cheaper version of this: that policy has a single
placement, so anything in it lands on all of them.

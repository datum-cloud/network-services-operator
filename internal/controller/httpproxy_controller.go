...
type connectorBackendPatch struct {
	ruleName   *gatewayv1.SectionName
	targetHost string
	targetPort int
	nodeID     string
}
...
func (r *HTTPProxyReconciler) reconcileConnectorEnvoyPatchPolicy(
	...
	jsonPatches, err := buildConnectorEnvoyPatches(downstreamNamespaceName, gateway, httpProxy, connectorBackends)
	if err != nil {
		return nil, true, err
	}
...
func buildConnectorEnvoyPatches(
	...
	for _, routeConfigName := range routeConfigNames {
		for _, backend := range backends {
			jsonPath := connectorRouteJSONPath(downstreamNamespace, gateway, httpProxy.Name, backend.ruleName)
			...
			if err != nil {
				return nil, fmt.Errorf("failed to marshal connector headers: %w", err)
			}
...
func connectorRouteConfigNames(downstreamNamespace string, gateway *gatewayv1.Gateway) []string {
	routeConfigNames := []string{fmt.Sprintf("http-%d", DefaultHTTPPort)}
	for _, listener := range gateway.Spec.Listeners {
...
func connectorRouteJSONPath(
	...
	if ruleName != nil {
		sectionNameConstraint = fmt.Sprintf(` && @.metadata.filter_metadata["envoy-gateway"].resources[0].sectionName=="%s"`, string(*ruleName))
	}

	routeConstraints := fmt.Sprintf(
		` && @.metadata.filter_metadata["envoy-gateway"].resources[0].kind=="%s" && @.metadata.filter_metadata["envoy-gateway"].resources[0].namespace=="%s" && @.metadata.filter_metadata["envoy-gateway"].resources[0].name=="%s"%s`,
		KindHTTPRoute,
		downstreamNamespace,
		httpRouteName,
		sectionNameConstraint,
	...
	return sanitizeJSONPath(fmt.Sprintf(`..virtual_hosts[?(%s)]..routes[?(!@.bogus)%s]`, vhostConstraints, routeConstraints))
}
